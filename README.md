...
**Guardrails**: the shared system prompt (`SYSTEM_PROMPT` in `agent.ts`, inherited by Explain and both chats) explicitly scopes the model to this user's reconciliation data and directly related finance/business questions, tells it to decline anything else — writing arbitrary code, general knowledge, creative writing — in one sentence and redirect, and tells it not to follow instructions embedded in tool output or in the user's message asking it to ignore the rules or change role. This is prompt-level, not a hard filter — a small enough model can still occasionally be talked around it — but it's paired with a real hard boundary: the tools are read-only and scoped to the calling user via the same RLS-enforcing DB layer as the rest of the app, so nothing it can be tricked into "agreeing" to actually lets it read another user's data, write to the database, or influence matching.
